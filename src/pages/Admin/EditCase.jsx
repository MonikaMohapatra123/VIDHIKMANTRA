...
useEffect(()=>{ fetch(); }, []);
async function fetch(){ try{ const r = await axios.get(`${BACKEND}/api/casestudies/${id}`); const data = r.data || {}; data.tags = (data.tags || []).join(', '); setValues(data); }catch(e){ alert('Load failed'); } finally{ setLoading(false); } }
function onChange(k,v){ setValues(prev=>({ ...prev, [k]: v })); }


async function handleSave(){ if(!values.title || !values.summary) return alert('Title and summary required'); setSaving(true); try{ const payload = { ...values, tags: (values.tags||'').split(',').map(t=>t.trim()).filter(Boolean) }; await axios.put(`${BACKEND}/api/casestudies/${id}`, payload); nav('/admin/casestudies'); }catch(e){ alert('Save failed'); } finally{ setSaving(false); } }
