...
export default function EditBlog() {
  ...
  // FIX: inline async function inside useEffect (no dependencies required)
  useEffect(() => {
    async function loadBlog() {
      try {
        const r = await axios.get(`${BACKEND}/api/blogs/${id}`);
        setValues(r.data || {});
      } catch (e) {
        alert("Load failed");
      } finally {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await axios.put(`${BACKEND}/api/blogs/${id}`, values);
      nav("/admin/blogs");
    } catch (e) {
      alert("Save failed");
    } finally {
